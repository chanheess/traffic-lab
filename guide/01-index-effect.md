...
|------|------|
| OS | macOS |
| Docker | MySQL 8.4 / Redis 7.4 |
| CPU | 1코어 (Spring Boot + MySQL 공유) |
| MySQL 버퍼풀 | 256MB (`innodb-buffer-pool-size=256M`) |
| MySQL 최대 커넥션 | 200 (`max-connections=200`) |
| 데이터 | `posts` 테이블 100,000건 |
| 동시 스레드 | 50개 |
| 측정 방식 | 전원 준비 후 동시 출발 (`CountDownLatch`) |

---
**테스트 결과**
> 100,000건에서 아래와 같은 개선 결과가 나온다.  
...
├──────────┼──────────────┼──────────────┼────────────┤
│ 평균     │        606ms │         18ms │     33.7x │
│ p50      │        616ms │         19ms │     32.4x │
│ p95      │        885ms │         23ms │     38.5x │
│ p99      │        887ms │         24ms │     37.0x │
│ 최대     │        887ms │         24ms │     37.0x │
└──────────┴──────────────┴──────────────┴────────────┘